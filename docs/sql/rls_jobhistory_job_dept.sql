...
CREATE POLICY "jh_insert" ON jobhistory FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM "UserModule_Rights" WHERE "user_id"=auth.uid() AND module_name='JH_Mod' AND add_right=1)
);

CREATE POLICY "jh_update_edit" ON jobhistory FOR UPDATE USING (record_status='ACTIVE')
WITH CHECK (EXISTS (SELECT 1 FROM "UserModule_Rights" WHERE "user_id"=auth.uid() AND module_name='JH_Mod' AND edit_right=1));

CREATE POLICY "jh_update_deactivate" ON jobhistory FOR UPDATE USING (true)
WITH CHECK (NEW.record_status='INACTIVE' AND EXISTS (SELECT 1 FROM "UserModule_Rights" WHERE "user_id"=auth.uid() AND module_name='JH_Mod' AND del_right=1));
...
CREATE POLICY "job_insert" ON job FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM "UserModule_Rights" WHERE "user_id"=auth.uid() AND module_name='Job_Mod' AND add_right=1)
);
CREATE POLICY "job_update_edit" ON job FOR UPDATE USING (record_status='ACTIVE')
WITH CHECK (EXISTS (SELECT 1 FROM "UserModule_Rights" WHERE "user_id"=auth.uid() AND module_name='Job_Mod' AND edit_right=1));

CREATE POLICY "job_update_deactivate" ON job FOR UPDATE USING (true)
WITH CHECK (NEW.record_status='INACTIVE' AND EXISTS (SELECT 1 FROM "UserModule_Rights" WHERE "user_id"=auth.uid() AND module_name='Job_Mod' AND del_right=1));
...
CREATE POLICY "dept_insert" ON department FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM "UserModule_Rights" WHERE "user_id"=auth.uid() AND module_name='Dept_Mod' AND add_right=1)
);
CREATE POLICY "dept_update_edit" ON department FOR UPDATE USING (record_status='ACTIVE')
WITH CHECK (EXISTS (SELECT 1 FROM "UserModule_Rights" WHERE "user_id"=auth.uid() AND module_name='Dept_Mod' AND edit_right=1));

CREATE POLICY "dept_update_deactivate" ON department FOR UPDATE USING (true)
WITH CHECK (NEW.record_status='INACTIVE' AND EXISTS (SELECT 1 FROM "UserModule_Rights" WHERE "user_id"=auth.uid() AND module_name='Dept_Mod' AND del_right=1));
